...
const volumeUrl = "https://min-api.cryptocompare.com/data/exchange/histoday?tsym=USD&limit=2000&&api_key=86cf65ef09473df5964e859710cbccbb0c12284cd4e5493023133a9f5035429f";
...
xhReq.open("GET", volumeUrl, false);
xhReq.send(null);
let volumeData = JSON.parse(xhReq.responseText)["Data"];

// convert Unix timestamps to Date objects in the volume data
for (let i = 0; i < volumeData.length; i++) {
  volumeData[i]["time"] = new Date(volumeData[i]["time"] * 1e3);
  // replace the huge numbers that sometimes appear in the data with the mean
  volumeData[i]["volume"] = (volumeData[i]["volume"] > 61211656445 ? 3481431966 : volumeData[i]["volume"])
}
...
const drawChart = response => {
  ...
  svg.append("g")
    ...
    .attr("x", d => xVolume(d.time))
    .attr("width", xVolume.bandwidth())
    .attr("y", d => yVolume(d.volume))
    .attr("height", d => height / 3 - yVolume(d.volume));
...
if (localStorage.getItem("startDate") && localStorage.getItem("endDate")) {
  d3.json(`${baseUrl}?start=${localStorage.getItem("startDate")}&end=${localStorage.getItem("endDate")}`)
    .then(drawChart);
} else {
  d3.json(baseUrl)
    .then(drawChart);
...
if (localStorage.getItem("startDate") && !localStorage.getItem("endDate")) {
  d3.json(`${baseUrl}?start=${localStorage.getItem("startDate")}&end=${endDate}`)
    .then(drawChart);
}

if (!localStorage.getItem("startDate") && localStorage.getItem("endDate")) {
  d3.json(`${baseUrl}?start=${startDate}&end=${localStorage.getItem("endDate")}`)
    .then(drawChart)
}
...
startDateInput.addEventListener("change", e => {
  ...
  d3.json(`${baseUrl}?start=${startDate}&end=${endDate}`)
    .then(drawChart);
});
